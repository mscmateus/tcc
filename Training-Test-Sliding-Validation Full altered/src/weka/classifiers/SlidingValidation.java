...
public class SlidingValidation {
    ...
    public void escritor(String linha) throws IOException {
    	String conteudo = "",conteudoAtual="";
	    	BufferedReader buffRead = new BufferedReader(new FileReader(System.getProperty("user.home")+"\\resultado.txt"));
	        while (true) {
	            if (conteudo != null) {
	            	if(!conteudo.equals(""))
	                ...
	                break;
	            conteudo = buffRead.readLine();
	        }
	        buffRead.close();
			// TODO: handle exception
    		BufferedWriter buffWrite = new BufferedWriter(new FileWriter(System.getProperty("user.home")+"\\resultado.txt"));
    		buffWrite.append(conteudoAtual+linha);
    		buffWrite.close();
    }
	...
	public String toClassifyMetricsString(int totalModels, int passo) throws Exception{
		StringBuffer text = new StringBuffer();
...
//		double minPredict = Double.MAX_VALUE;
		...
		for (int i = 0; i <testInstances.numInstances(); i++){
			...
			//provavel ssr - jonathan testar sem m·scara
...
//			sumSquaredResiduals += residuals[i];
		...
		//varrer residuals[i] e somar (calculo do r^2 - regressionanalysis classe weka - calcular RSquared)
	...
	//jonathan
	public String toRegressionMetricsStringJon(int totalModels){
		StringBuffer text = new StringBuffer();
		//double R2Avg = somaR2/count;
